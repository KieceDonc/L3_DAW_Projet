...
function showCourses(){
  ...
  if(count($courses) == 0){
    echo "<p> You didn't created a course yet. To do so, please see <a href='/admin/courseBuilder'> course builder page </a></p>";
  }
  else{
    // Only displays for now as a test. TODO : Show courses as link that take the user to the course page.
    foreach($courses as $course){
      echo "<div class='courses'><span class='courseName'>" . $course["name"] . "</span> par " . $course["username"] . "<br>Description : " . $course["description"] . "</div>";
    }
  }
}
